      ...
      λ(talks : Section) →
        ''
            ...
            <body>
                ...
                var PER_PX = 0.055 / 24;
                var CELL = 10, SCALE = PER_PX * CELL;
                var LEVELS = 7, LO = 0.26, HI = 0.62;   // fitted to the field's actual range

                var canvas = document.getElementById("field");
                var ctx = canvas.getContext("2d");
                var cols = 0, rows = 0, dpr = 1, grid = null, z = 0;
                ...
                function draw(){
                    ...
                    for (gy = 0; gy < rows - 1; gy++){
                        ...
                        var ii = gy * cols + gx;
                        var a = grid[ii], b = grid[ii + 1], c = grid[ii + cols + 1], d = grid[ii + cols];
                        ...
                        switch (idx){
                            ...
                            case 5:
                            if ((a + b + c + d) / 4 > t){ seg(left, top); seg(bottom, right); }
                            else { seg(left, bottom); seg(top, right); }
                            break;
                            case 10:
                            if ((a + b + c + d) / 4 > t){ seg(left, bottom); seg(top, right); }
